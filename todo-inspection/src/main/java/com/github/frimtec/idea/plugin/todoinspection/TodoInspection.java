...
import static com.github.frimtec.idea.plugin.todoinspection.OptionDialogHelper.enumOption;
...
public class TodoInspection extends LocalInspectionTool {
  ...
  @NonNls
  public ApiVersion apiVersion = ApiVersion.V2;
  ...
  private final List<OptionDialogHelper.Option> options = List.of(
      ...
      enumOption(
          "Jira API version",
          "The Jira API version supported by the Jira instance.",
          () -> this.apiVersion,
          (value) -> {
      ...
      action("Test connection", button -> {
        ...
        try {
          TodoScanner scanner = new TodoScanner(createJiraService(), split(inspectionOptions.jiraProjectKeys()), Ticket.statusMapper(Set.of()));
          boolean isConnectionSuccessful = scanner.testConnection();
  ...
  public void writeSettings(@NotNull Element node) {
    ...
    this.apiVersion = this.inspectionOptions.apiVersion();
  ...
  private @NotNull @InspectionMessage String formatMessage(Todo todo) {
    return switch (todo.status()) {
      ...
      case INCONSISTENT_TICKET_NOT_EXISTING -> "%s references a ticket that does not exist".formatted(todo.type());
      case NO_TICKET_REFERENCE -> "%s does not reference a ticket".formatted(todo.type());
      case UNKNOWN_TICKET_STATUS -> "%s references a ticket for which the ticket status is currently unknown".formatted(todo.type());
